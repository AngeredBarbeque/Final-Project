...
def collision(map, p):
    ...
    if get_block({"x": p["x_pos"], "y": p["y_pos"]-1}, p, map) in ["█", "▓", "|"]:
        Add 'down' to collision returns
    if get_block({"x": p["x_pos"], "y": p["y_pos"]+1}, p, map) is a block or a falling block:
        Add 'up' to collision returns
    if get_block({"x": p["x_pos"]-1, "y": p["y_pos"]}, p, map) is a block or a falling block:
        Add 'left' to collision returns
    if get_block({"x": p["x_pos"]+1, "y": p["y_pos"]}, p, map) is a block or a falling block:
        Add 'right' to collision returns
